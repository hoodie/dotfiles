...
prompt_git() {
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    if [[ -n $dirty ]]; then
      prompt_segment yellow NONE
    else
      prompt_segment green NONE
    fi
...
prompt_dir() {
  prompt_segment NONE blue '%B%c%b'
}
...
prompt_virtualenv() {
  ...
  if [[ -n $virtualenv_path && -n $VIRTUAL_ENV_DISABLE_PROMPT ]]; then
    prompt_segment blue NONE "(`basename $virtualenv_path`)"
  fi
}
